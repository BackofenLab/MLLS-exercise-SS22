...
def create_training_proc(compact_data, driver_ids, passenger_ids, gene_features, mapping):
    ...
    x, edge_index = compact_data.x, compact_data.edge_index
    explainer = GNNExplainer(model, epochs=200, num_hops = 1)
    ...
    for i in [1, 10, 20]:
        ...
        thresh = torch.quantile(edge_mask, 0.8)
        
        ax, G = explainer.visualize_subgraph(node_idx, edge_index = edge_index, edge_mask = edge_mask, y=compact_data.y, threshold = thresh.item())
        plt.savefig("./subgraph_thresh_" +str(i) + ".svg")
        plt.close()
...
def predict_data(model, compact_data, test_driver_genes, test_passenger_genes):
    ...
    dr_cls_acc, auprc_wt, dr_fpr, dr_tpr, dr_precision, dr_recall, dr_roc_auc_score = agg_per_class_acc(out, pred, compact_data, test_driver_genes, test_passenger_genes)
...
def plot_dr_prec_recall(fpr_epo, tpr_epo, dr_prec_epo, dr_recall_epo, dr_roc_auc_score, auprc_wt):
    ...
    plt.grid(True)
    plt.savefig("./cancer_{}_prc_all_drivers_links_{}_epo_{}.pdf".format(cancer_type, n_edges, n_epo), dpi=200)
    plt.show()
    ...
    plt.grid(True)
    plt.savefig("./cancer_{}_roc_all_drivers_links_{}_epo_{}.pdf".format(cancer_type, n_edges, n_epo), dpi=200)
    plt.show()
...
def plot_loss_acc(n_epo, tr_loss, te_acc):
    ...
    plt.savefig("./cancer_{}_{}_fold_CV_driver_training_loss_links_{}_epo_{}.pdf".format(cancer_type, k_folds, n_edges, n_epo), dpi=200)
    plt.show()
    ...
    plt.savefig("./cancer_{}_{}_fold_CV_driver_test_accuracy_links_{}_epo_{}.pdf".format(cancer_type, k_folds, n_edges, n_epo), dpi=200)
    plt.show()
